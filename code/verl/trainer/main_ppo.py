...
def run_ppo(config) -> None:
    if not ray.is_initialized():
        ...
        ray.init(
            ...
            # runtime_env={"env_vars": {"TOKENIZERS_PARALLELISM": "true", "NCCL_DEBUG": "WARN", "VLLM_LOGGING_LEVEL": "WARN", "VLLM_ALLOW_RUNTIME_LORA_UPDATING": "true", "RAY_DEBUG_POST_MORTEM": "1"}},
...
class TaskRunner:
    def run(self, config):
        ...
        reward_fn = reward_manager_cls(tokenizer=tokenizer, num_examine=0, normalize_by_length=False, env_name=config.env.env_name)

        # Note that we always use function-based RM for validation
        val_reward_fn = reward_manager_cls(tokenizer=tokenizer, num_examine=1, normalize_by_length=False, env_name=config.env.env_name)
