...
| A.2 | **Framework base activado: fork de `CharacterRL-iStar`** (Apache-2.0, veRL fork de Alibaba Tongyi, ICLR 2026). Modificación estimada: 30-100 líneas — reemplazar el cómputo del término privilegiado del log-ratio (PRM aprendido vía DPO → mismo modelo con golden en prompt). | 🔒 Cerrada (2026-05-11) | Decisión tomada al cerrar [#14](https://github.com/lucaspecina/piar-rl/issues/14). Verificación operacional 2026-05-11: 7 trainers ejecutables (iStar, RLOO, GRPO, REINFORCE++, PPO, GiGPO, PRIME) × 2 environments (WebShop, Sokoban). Qwen2.5-7B-Instruct base, 8×H100/A100 single-node. Hyperparams loggeados en `examples/istar_trainer/run_webshop.sh`. La reformulación de PROJECT.md ("Reformulación operativa contra iStar") apunta directo a este codebase como base+baseline. | [#14](https://github.com/lucaspecina/piar-rl/issues/14) |
