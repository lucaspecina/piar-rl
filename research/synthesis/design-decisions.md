...
| C.5 | **Privileged context** = solución completa con CoT, no answer literal solo. La forma específica para PIAR (CoT vs SCM vs trace exitoso) merece su propia ablation por el riesgo de leakage. | 🟡 Abierta | OPSD usa CoT completo (`paper-opsd.md` §2). π-Distill (`paper-pi-distill.md` §2) provee taxonomía concreta de PI types: **Tool Calls & Arguments** (denso), **Tool Calls Only** (medio), **Self-Generated Hints** (bajo). Tool Calls & Args gana en sus benchmarks; PI menos denso requiere α>0 (teacher training) para funcionar bien. **Para PIAR (teacher frozen)**: empezar con PI denso (golden + CoT estilo OPSD), explorar variantes menos densas (SCM, hints) como ablation. | [#5](https://github.com/lucaspecina/piar-rl/issues/5), [#10](https://github.com/lucaspecina/piar-rl/issues/10) |
